...
use cfg::DbCfg;
use clap::Parser;
use std::{io::{Read, Write}, net::{IpAddr, SocketAddr}, path::PathBuf, sync::Arc};
...
fn main() {
    ...
    let cfg_path = match args.config { 
        Some(cfgp) => if cfgp.is_file() {cfgp} 
            else {
    ...
    let addr: std::net::SocketAddr = match ncfg.api_addres_and_port.parse() { 
        Ok(r) => r, 
        Err(_) => { panic!("failed to parse host addrss")}
    };
    
    axum::serve(tokio::net::TcpListener::bind(addr).await.unwrap(), api_router).await.unwrap();          
    })
